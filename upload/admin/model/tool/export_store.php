...
class ModelToolExportStore extends Model {
	...
	public function setCell(&$worksheet, $row/*1-based*/, $col/*0-based*/, $val, $style = null) {
		if ($style) {
	...
	public function loadSheet($filename, $sheetname) {
		...
		$sheetname = utf8_strtolower($sheetname);
		...
		switch ($sheetname) {
			...
			case 'additionalimages':
				if ( !$this->validateAdditionalImages( $reader ) ) {
					error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('error_images_header')."\n",3,DIR_LOGS."error.txt");
					return false;
				}
				break;
			case 'options':
			...
			case 'rewards':
				...
				break;
			default:
				error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('error_sheetname_header')."\n",3,DIR_LOGS."error.txt");
				return false;
				break;
		}
	...
	public function getDefaultLengthUnit() {
		return $this->length->getUnit($this->config->get('config_length_class_id'));
	}
	...
	public function getLayoutIds() {
		...
		$layout_query = $this->db->query( "SELECT * FROM `" . DB_PREFIX . "layout`" );
	...
	public function storeAdditionalImagesIntoDatabase(&$images) {
		...
		foreach ($images as $image) {
			...
			$sql .= ($first) ? "\n" : ",\n";
			$first = false;
			$sql .= "($productImageId,$productId,'".$this->db->escape($imageName)."',$sortOrder)\n";
		}
		if (!$first) {
			$this->db->query($sql);
		}
	...
	public function storeAttributesIntoDatabase(&$attributes) {
		...
		foreach ($attributes as $attribute) {
			...
			if (!isset($attributeGroupIds[$group])) {
				...
				$sql .= "($attributeGroupId,$langId,'".$this->db->escape($group)."');";
				$this->db->query( $sql );
			}
			if (!isset($attributeIds[$group])) {
				$attributeIds[$group] = array();
			...
			if (!isset($attributeIds[$group][$name])) {
				...
				$sql .= "($attributeId,$langId,'".$this->db->escape($name)."');";
				$this->db->query( $sql );
			}
			...
			$sql .= "($productId,$attributeId,$langId,'".$this->db->escape( $text )."');";
			$this->db->query( $sql );
		}
...
//		$sql .= "DELETE FROM `".DB_PREFIX."url_alias` WHERE `query` LIKE 'category_id=%';\n";
		...
		$availableStoreIds = $this->getStoreIds();
	...
	public function storeDiscountsIntoDatabase(&$discounts) {
		...
		foreach ($discounts as $discount) {
			...
			if (!isset($customerGroups[$name])) {
				...
				$sql .= "($maxCustomerGroupId, $languageId, '".$this->db->escape($name)."', '' )";
				$sql .= ";\n";
				$this->db->query($sql);
	...
	public function storeOptionsIntoDatabase(&$options) {
		...
		foreach ($options as $option) {
			...
			if (!isset($optionIds[$name][$type])) {
				...
				$this->db->query( $sql );
				$sql = "INSERT INTO `".DB_PREFIX."option_description` (`option_id`,`language_id`,`name`) VALUES ($optionId,$langId,'".$this->db->escape($name)."');";
				$this->db->query( $sql);
			}
			if (($type=='select') || ($type=='checkbox') || ($type=='radio') || ($type=='image')) {
				...
				if (!isset($optionValueIds[$name][$type][$value][$image])) {
					...
					$sql .= "($optionValueId,$optionId,'".$this->db->escape($image)."',$sortOrder);";
					$this->db->query( $sql );
					$sql  = "INSERT INTO `".DB_PREFIX."option_value_description` (`option_value_id`,`language_id`,`option_id`,`name`) VALUES ";
					$sql .= "($optionValueId,$langId,$optionId,'".$this->db->escape($value)."');";
					$this->db->query( $sql );
				} else {
					$optionValueId = $optionValueIds[$name][$type][$value][$image];
					$sortOrder = ($option['sort_order'] == '') ? 0 : $option['sort_order'];
					$optionId = $optionIds[$name][$type];
					$sql  = "UPDATE `".DB_PREFIX."option_value` SET `image`='".$this->db->escape($image)."', `sort_order`=$sortOrder ";
			...
			if (!isset($productOptionIds[$productId][$optionId])) {
				...
				$sql .= "($productOptionId,$productId,$optionId,'".$this->db->escape($productOptionValue)."',$required);";
				$this->db->query( $sql );
			}
			if (($type=='select') || ($type=='checkbox') || ($type=='radio') || ($type=='image')) {
...
//		$sql .= "DELETE FROM `".DB_PREFIX."url_alias` WHERE `query` LIKE 'product_id=%';\n";
		...
		foreach ($products as $product) {
			...
			$productName = $this->db->escape($product['name']);
			...
			$productDescription = $this->db->escape($product['description']);
			...
			$meta_keywords = $this->db->escape($product['meta_keywords']);
			$tags = $this->db->escape($product['tags']);
...
*/
	...
	public function uploadAdditionalImages(&$reader) {
		...
		return $this->storeAdditionalImagesIntoDatabase( $images );
	}
	...
	public function uploadProducts($storeId, &$reader) {
		...
		$defaultMeasurementUnit = $this->getDefaultLengthUnit();
	...
	public function uploadRewards(&$reader) {
		...
		return $this->storeRewardsIntoDatabase( $rewards );
	}
...
return true;
			...
			$data = $this->loadSheet($filename, 'Options');
			if (!$this->uploadOptions( $data )) {
				return false;
			}

			$data = $this->loadSheet($filename, 'Attributes');
			if (!$this->uploadAttributes( $data )) {
				return false;
			}

			$data = $this->loadSheet($filename, 'Specials');
			if (!$this->uploadSpecials( $data )) {
				return false;
			}

			$data = $this->loadSheet($filename, 'Discounts');
			if (!$this->uploadDiscounts( $data )) {
				return false;
			}

			$data = $this->loadSheet($filename, 'Rewards');
			if (!$this->uploadRewards( $data )) {
				return false;
			}
			return true;
		} catch (Exception $e) {
